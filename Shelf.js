...
Shelf.prototype.makeFlatShelf = function () {
	...
	var flatGeom = new THREE.CubeGeometry(this.width, this.flatThickness, this.depth);
...
Shelf.prototype.addBooks = function (numBooks, scene, objects, objectsControls) {
	...
	for (var i = 0; i < numBooks; i++) {
		...
		var bookDepth = this.width/numBooks;
		var bookSpacing = 0.3 * bookDepth;
		var book = new Book(bookWidth, bookHeight, (Math.random() * 0.5 + 0.5) * (bookDepth - bookSpacing), Math.random() * 0xffffff, 'book' + i);
		var object = book.frame;
		object.position.copy(shelfPos);
		object.position.x -= 0.5 * this.width;
		object.position.x += (i + 0.75) * bookDepth;
		object.position.y += this.flatThickness + 0.5 * bookHeight;
